...
def _default_relay_path():
    """Locate the canonical Neutral Relay.

    Priority:
      1. GOVERLOOP_RELAY_PATH env override
      2. sibling-of-repo layout (this script lives in <repo>/tools/session-manager/)
      3. legacy WorkBuddy skill checkout layout (kept for existing installs)
    """
    env = os.environ.get("GOVERLOOP_RELAY_PATH")
    if env:
        return os.path.expanduser(env)
    here = os.path.dirname(os.path.abspath(__file__))
    repo = os.path.abspath(os.path.join(here, "..", ".."))
    rel = os.path.join(repo, "tools", "neutral-relay", "neutral_relay.py")
...
RELAY_DEFAULT = _default_relay_path()
...
def relay_supports_attachment(relay):
    ...
    The relay's argparse interface has changed over time; passing an unknown
    flag makes argparse exit 2 (CHECKPOINT_DELIVERY_INCOMPLETE). Fail open on
    probe errors so callers degrade to inline delivery.
    """
    try:
        r = subprocess.run(
...
def run_checkpoint(
    ...
    # relay capability probe: newer relay versions dropped --attachment support.
    # Degrade to inlining the attachment text into the message body so the
    # conversation still receives the full content (honest degradation, never a
    # false "attachments delivered").
