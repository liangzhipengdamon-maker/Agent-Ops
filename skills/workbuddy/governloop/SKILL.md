...
```bash
python3 <repo>/tools/session-manager/governloop_session.py <subcommand> [args]
```
...
- `GOVERLOOP_RELAY_PATH` — path to `neutral_relay.py` (default: auto-located
  in the same repo under `tools/neutral-relay/`, with a legacy install
  fallback)
...
See `docs/QUICK_START.md` for the user-facing guide, `docs/USAGE.md` for the
full reference, `docs/MULTI_PROJECT_WORKFLOW.md` for cross-project isolation,
and `references/policy.md` for the full session-routing / checkpoint /
